...
int main(){
	...
	err1 = pthread_create(&thread1,NULL,matrix_sub,mesg1);
	if(err1!=0){
		exit(1);
	}
	else{
	...
	err2 = pthread_create(&thread2,NULL,matrix_sub,mesg2);
	if(err2!=0){
		exit(1);
	}
	else{
...
//	printf("\n%s",flag);
	...
	if(strcmp(my_work,"isEven")){
		printf("\nThread #1 : Subtracting even rows");
